...
preferences {
    section("Allow IOTDB to Control & Access These Things...") {
		...
		input "d_garage", "capability.doorControl", title: "Garage Door", required: false, multiple: true
    }
}
...
def _event_subscribe()
{
    ...
    subscribe(d_garage, "door", "_on_event")
}
...
private _device_to_json(device, type) {
    ...
    if (type == "switch") {
        ...
        def s = device.currentState('garageDoorControl')
        vd['timestamp'] = s?.isoDate
        vd['garageDoorControl'] = s?.value == "closed"
    }
