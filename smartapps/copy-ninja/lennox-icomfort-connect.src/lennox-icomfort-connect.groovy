	...
	apiGet("/DBAcessService.svc/GetTStatInfoList", [query: [GatewaySN: getDeviceGatewaySN(device), TempUnit: (getTemperatureScale()=="F")?0:1, Cancel_Away: "-1"]]) { response ->
		if (response.status == 200) {
			response.data.tStatInfo.each { 
                ...
                update_openhab(it.Indoor_Temp,it.Indoor_Humidity,it.Cool_Set_Point,it.Heat_Set_Point,lookupInfo( "thermostatOperatingState", it.System_Status.toString(), true ),lookupInfo( "thermostatMode", it.Operation_Mode.toString(), true ))
			}
...
def update_openhab(temp_data,humid_data,coolsp,heatsp,ts_state,ts_mode) {
	
	def request = [
    method: "PUT",
    path: "/rest/items/tstat_temp/state",
    headers: [
   		HOST: "192.168.1.5:80",
	...
	request = [
    method: "PUT",
    path: "/rest/items/tstat_humid/state",
    headers: [
   		HOST: "192.168.1.5:80",
        'Content-Type': "text/plain"
		],
    ...
    request = [
    method: "PUT",
    path: "/rest/items/tstat_coolsp/state",
    headers: [
   		HOST: "192.168.1.5:80",
        'Content-Type': "text/plain"
		],
    ...
    log.debug "Updated Temp Set Point Data for Openhab temp=$temp_data humid=$humid_data"
    
    request = [
    method: "PUT",
    path: "/rest/items/tstat_heatsp/state",
    headers: [
   		HOST: "192.168.1.5:80",
        'Content-Type': "text/plain"
		],
    ...
    request = [
    method: "PUT",
    path: "/rest/items/tstat_state/state",
    headers: [
   		HOST: "192.168.1.5:80",
        'Content-Type': "text/plain"
		],
	...
	catch (e)
	{
        ...
        request = [
    method: "PUT",
    path: "/rest/items/tstat_mode/state",
    headers: [
   		HOST: "192.168.1.5:80",
        'Content-Type': "text/plain"
		],
    ...
    log.debug "Updated Temp Set Point Data for Openhab temp=$temp_data humid=$humid_data heat=$heatsp cool=$coolsp state=$ts_state mode=$ts_mode"
}