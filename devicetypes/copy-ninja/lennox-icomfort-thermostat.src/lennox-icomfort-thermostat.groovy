...
def updateThermostatData(thermostatData) {		
	...
	//update_openhab(device.currentValue("temperature"),device.currentValue("humidity"))
...
def update_openhab(temp_data,humid_data) {
    ...
    try {
    	def myhubAction =new physicalgraph.device.HubAction(request)
    ...
    log.debug "Updated Tstat Data for Openhab temp=$temp_data humid=$humid_data"
}